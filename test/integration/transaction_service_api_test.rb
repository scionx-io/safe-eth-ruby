...
class TransactionServiceApiTest < Minitest::Test
  ...
  def test_incomings_transfers
    response = @api.incomings_transfers(address: "0x48F945aafB38658243d38eEb89538e879fba4781", options: {})
    assert_kind_of(Hash, response)
    assert_kind_of(Array, response["results"])
    refute_empty(response["results"])
  end

  def test_module_transactions
    response = @api.module_transactions(address: "0x48F945aafB38658243d38eEb89538e879fba4781", options: {})
    assert_kind_of(Hash, response)
    assert_kind_of(Array, response["results"])
  end
